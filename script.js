...
// Only run if normal search section exists
const normalSection = document.getElementById('normal-search-section');
if (normalSection && !document.getElementById('wordbomb-toggle')) {
    
    // Create Word Bomb toggle UI
    const wordbombHTML = `
        <div class="wordbomb-container" style="display: flex; align-items: center; gap: 20px; margin: 15px 0; padding: 15px 20px; background: var(--bg-secondary); border-radius: 8px; border: 1px solid var(--border-primary);">
            <div style="display: flex; align-items: center; gap: 10px;">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 500; color: var(--text-primary);">
                    <input type="checkbox" id="wordbomb-toggle" style="width: 18px; height: 18px; cursor: pointer; accent-color: var(--text-primary);">
                    <span>💣 Word Bomb Mode</span>
                </label>
            </div>
            <div id="wordbomb-hint" style="color: var(--text-secondary); font-size: 0.9rem; font-style: italic;">
                Find words that start with prefix AND end with suffix
            </div>
        </div>
    `;
    ...
    if (searchFilters) {
        searchFilters.insertAdjacentHTML('afterend', wordbombHTML);
    }
    
    // Store references to the searcher methods
    const originalPerformSearch = searcher.performSearch.bind(searcher);
    const originalDisplayResults = searcher.displayResults.bind(searcher);
    const originalShowError = searcher.showError.bind(searcher);
    
    // Override performSearch with Word Bomb support
    searcher.performSearch = function() {
        ...
        if (wordBombMode) {
            ...
            // Combine prefix and suffix as a single pattern
    ...
    document.getElementById('wordbomb-toggle')?.addEventListener('change', function() {
        ...
        if (this.checked) {
            prefixInput.placeholder = 'Enter first part...';
            suffixInput.placeholder = 'Enter second part...';
            ...
            hint.innerHTML = 'Find words that start with prefix AND end with suffix';
        }
    });
    
    // Override clearSearch to reset word bomb
    ...
    console.log('💣 Word Bomb Mode added!');
}
})();
